...
export default {
  ...
  methods: {
    ...
    // SET_SAVE_SELECT_SUBJECT 设置当前选择的学段
    ...
    getSelect (val) {
      ...
      this.searchCode.knowledgeId = val[0].oldId // 默认显示章节树ID
    },
    // 试题排序
    getSearchData (e, i) {
    ...
    // 设置章节树
    setChapter (val) {
      // console.log(val.grade.list[this.searchCode.GradeCode].code)
      if (val.grade.list.length) {
        // eslint-disable-next-line no-unused-vars
        const url = 'http://udog.beautop.cn:33833/gdy/question_bank/xukubao_api?s=Index&m=Api&a=chapterApi&accessKey=37f600e9c80494c20d950d3081e497e5' // 绑定账号登录时的学段、科目
        this.$axios.get(`${url}
          ...
          .then(({ data }) => {
            ...
            switch (data.errorCode) {
              ...
              case '4':
                ...
                break
              default:
                console.log(data, '教材树接口返回data-------------------------------')
                this.$store.dispatch('SAVE_TREE_MATERIAL', data.data) // 设置章节树
                break
            }
          })
    ...
    getData () {
      this.navTitleList = []
      // 数据遍历
      this.setDataTraversal(this.base_interface_data_period)
    },
    // 根据章节获取试题列表
    getQuestions () {
